...
class NerPredicter:
    def __init__(self, vocab_file: str = 'vocab.json', model_path: str = 'model/ner/ner.pt'):
    ...
    def build_seg_row(self, length: int) -> torch.Tensor:
        row = torch.zeros((self.io_sequence_size,), dtype=torch.long)
        if length <= 0:
            return row
        if length == 1:
            row[0] = 0
    ...
    def predict(self, input_text: str) -> str:
        ...
        segs = self.build_seg_row(seq_len).unsqueeze(0).to(self.device)
        lengths = torch.tensor([seq_len], dtype=torch.long, device=self.device)
        with torch.no_grad():
            label_list = self.model.predict(inputs, lengths, seg_inputs=segs)
        taggs = [self.classids[idx] for idx in label_list[0][:seq_len]]
...
def main():
    ...
    parser.add_argument('--model-path', default='model/ner/ner.pt')
    parser.add_argument('--text', default='肯德基在哪里')
