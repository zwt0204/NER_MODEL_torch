...
class NerTrainner:
    ...
    def train(self, epochs: int, dstfile: str = 'train.json', devfile: str | None = None):
        ...
        for epoch in range(epochs):
            ...
            for batch in self.iter_batches(records):
                ...
                loss = self.model.loss(batch.inputs, batch.targets, batch.lengths, seg_inputs=batch.segs)
                loss.backward()
    ...
    def convert_batch(self, records: List[dict]) -> Batch:
        ...
        segs = torch.zeros((count, self.io_sequence_size), dtype=torch.long)
        ...
        for i, record in enumerate(records):
            ...
            segs[i] = self.build_seg_row(xlen)
    ...
    def evaluate(self, records: List[dict]):
        ...
        with torch.no_grad():
            for batch in self.iter_batches(records):
                pred_batch = self.model.predict(batch.inputs, batch.lengths, seg_inputs=batch.segs)
...
def main():
    ...
    tmp_dir = tempfile.mkdtemp(prefix='ner_idcnn_train_')
