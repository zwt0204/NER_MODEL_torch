...
class NerPredicter:
    def __init__(self, vocab_file: str = 'vocab.json', model_path: str = 'models/LatticeLSTM/ner.pt'):
    ...
    def predict(self, input_text: str) -> str:
        ...
        lex_ids, lex_lens = self.build_lexicon_features(input_text)
        lex_ids = lex_ids.unsqueeze(0).to(self.device)
        lex_lens = lex_lens.unsqueeze(0).to(self.device)
        lengths = torch.tensor([seq_len], dtype=torch.long, device=self.device)
        with torch.no_grad():
            label_list = self.model.predict(inputs, lengths, lex_ids, lex_lens)
