# -*- encoding: utf-8 -*-
"""
PyTorch simplified lattice-style BiLSTM-CRF baseline for NER.
This implementation does not reproduce the original custom TensorFlow LatticeLSTM cell.
Instead, it fuses per-position character features with aggregated lexicon features,
then applies BiLSTM + CRF for a practical torch migration baseline.
"""
...
class NerCore(nn.Module):
    def __init__(
        ...
        self.lexicon_embedding_size = 64
        self.hidden_size = 128
        self.num_layers = 1

        self.char_embedding = nn.Embedding(self.vocab_size, self.char_embedding_size, padding_idx=0)
        self.lexicon_embedding = nn.Embedding(lexicon_vocab_size, self.lexicon_embedding_size, padding_idx=0)
        self.lexicon_length_proj = nn.Linear(1, self.lexicon_embedding_size)
        self.dropout = nn.Dropout(1 - self.keep_prob)
        self.encoder = nn.LSTM(
            input_size=self.char_embedding_size + self.lexicon_embedding_size,
    ...
    def encode_lexicon(self, lexicon_ids: torch.Tensor, lexicon_lengths: torch.Tensor) -> torch.Tensor:
        # lexicon_ids: [B, T, K]
        lex_emb = self.lexicon_embedding(lexicon_ids.long())
        length_feat = self.lexicon_length_proj(lexicon_lengths.unsqueeze(-1).float())
        fused = lex_emb + length_feat
        valid = (lexicon_ids > 0).unsqueeze(-1)
        fused = fused.masked_fill(~valid, 0.0)
        denom = valid.sum(dim=2).clamp_min(1)
        return fused.sum(dim=2) / denom
    ...
    def forward(
        ...
        char_x = self.char_embedding(inputs.long())
        lex_x = self.encode_lexicon(lexicon_ids, lexicon_lengths)
        x = torch.cat([char_x, lex_x], dim=-1)
        x = self.dropout(x)
    ...
    def loss(
        ...
        emissions = self.forward(inputs, sequence_lengths, lexicon_ids, lexicon_lengths)
        mask = self.sequence_mask(sequence_lengths, inputs.size(1))
